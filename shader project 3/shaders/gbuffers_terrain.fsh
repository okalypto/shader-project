...
void main() {
	...
	vec2 lm = lmcoord;
		...
		color.rgb = color.rgb*(torch_color*lm.x
	...
	#if LIGHTING_STYLE == 2
		vec3 torch_color = vec3(TORCH_R,TORCH_G,TORCH_B);
		float lightDot = clamp(dot(normalize(shadowLightPosition), normals_face),0.,0.25);
		color.rgb = color.rgb*(torch_color*lm.x //torch light
			+texture2D(lightmap, lm).y*lm.y //sky light
			+lightDot //sun light
			);
		//shadowLightPosition
	#endif
